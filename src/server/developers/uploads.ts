...
import { DEVELOPER_DOCUMENTS } from '@/lib/developer-registration';
import type { DeveloperUploads } from '@/server/services/developer-registrations';

/** Reads a developer registration multipart body: string fields, and documents under `doc_<KIND>`. */
export async function readDeveloperUploads(req: NextRequest) {
  ...
  const uploads: DeveloperUploads = {};
  ...
  /** Every string field, for the route's schema. */

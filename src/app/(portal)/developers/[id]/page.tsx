import { cache } from 'react';
import type { Metadata } from 'next';
import { notFound } from 'next/navigation';
import { requirePageCapability } from '@/server/auth/page-guard';
import { CAPABILITIES } from '@/lib/constants';
import { getDeveloperRegistration } from '@/server/services/developer-registrations';
import { isApiError } from '@/server/http/errors';
import { serialize } from '@/server/http/serialize';
import { PageHeader } from '@/components/common/page-header';
import { DeveloperPanel } from '@/features/developers/panel';
import type { DeveloperDetailPayload } from '@/features/developers/types';

export const dynamic = 'force-dynamic';

const load = cache(async (user: Parameters<typeof getDeveloperRegistration>[0], id: string) => {
  try {
    return await getDeveloperRegistration(user, id);
  } catch (error) {
    if (isApiError(error) && (error.status === 404 || error.status === 403)) return null;
    throw error;
  }
});

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const user = await requirePageCapability(CAPABILITIES.DEVELOPER_VIEW);
  const data = await load(user, (await params).id);
  if (!data) notFound();
  return { title: data.registration.registrationNumber ?? data.registration.applicationNumber };
}

export default async function DeveloperDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const user = await requirePageCapability(CAPABILITIES.DEVELOPER_VIEW);
  const data = await load(user, (await params).id);
  if (!data) notFound();
  const r = data.registration;
  return (
    <div className="space-y-2">
      <PageHeader
        title={r.registrationNumber ? `Developer ${r.registrationNumber}` : `Developer registration ${r.applicationNumber}`}
        description={[r.applicationNumber, r.organization || r.developerName, r.authorizedPerson].filter(Boolean).join(' · ')}
      />
      <DeveloperPanel initial={serialize(data) as unknown as DeveloperDetailPayload} />
    </div>
  );
}

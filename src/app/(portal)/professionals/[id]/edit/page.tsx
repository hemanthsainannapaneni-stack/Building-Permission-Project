...
export const metadata: Metadata = { title: 'Edit professional registration' };
...
export default async function EditProfessionalPage({ params }: { params: Promise<{ id: string }> }) {
  const user = await requirePageCapability(CAPABILITIES.PROFESSIONAL_REG_REGISTER);
  ...
  if (!data) notFound();
  if (!data.permissions.edit.offered) redirect(`/professionals/${id}`);

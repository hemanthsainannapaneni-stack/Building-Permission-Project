...
export default async function ProfessionalsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  const user = await requirePageCapability(CAPABILITIES.PROFESSIONAL_REG_VIEW);
  ...
  return (
    <div className="space-y-3.5">
      <PageHeader
        title="Professionals"
        description={`Registration → review → shortfall / verification → approval → available for applications → renewal. ${summary.AVAILABLE} professional${summary.AVAILABLE === 1 ? ' is' : 's are'} approved and in force.`}
        actions={
          can(user, CAPABILITIES.PROFESSIONAL_REG_REGISTER) ? (
            <Button asChild>
      ...
      <nav aria-label="Registers" className="grid gap-2 sm:grid-cols-4 lg:grid-cols-8">
        {PROFESSIONAL_REGISTERS.map((r) => (
          <Link
            key={r}
            href={r === 'ALL' ? '/professionals' : `/professionals?register=${r}`}

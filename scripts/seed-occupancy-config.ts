/**
 * PHASE 10 CONFIGURATION ONLY — the grants and the workflow rows.
 *
 *   npm run occupancy:config
 *
 * For a database seeded before the occupancy module
 * existed. Runs the two idempotent core seeds it needs — the permission matrix
 * (adds OCCUPANCY_* and re-syncs every role to src/lib/rbac-matrix.ts)
 * and the workflow definitions (adds the nine occupancy actions and their transitions,
